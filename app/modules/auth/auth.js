...
exports.changePassword = (id, token, password) => {
    return new Promise((revoke, reject) => {
        httpModule
            .request({
                url: "https://damboy.sytes.net/mk/changePassword.php",
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                    ...
                    if (res.statusCode === 200) {
                        try {
                            ...
                            if (json.status === "success") {
                                ...
                                console.log("auth.js: 0x18", json);
                                reject(json.msg);
                            }
                        } catch (e) {
                            console.log("auth.js: 0x17", e, res.content);
                            reject("Server error...");
                        }
                    } else {
                        console.log("auth.js: 0x16");
                        reject("Server error...");
                    }
                },
                e => {
                    console.log("auth.js: 0x15");
                    reject("Server error...");
                }
            );
    });
